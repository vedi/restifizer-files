...
function FileDataService(options) {
  ...
  this._provider.initialize.apply(this._provider, arguments);
}
        ...
        if (fileMeta) {
          if (typeof(fileMeta) === 'object') {
            return Promise
              .bind(this)
              .then(function () {
                return this._provider.getFileAsync(fileMeta);
              })
              .then(function (file) {
                return [file, fileMeta.contentType];
              });
          } else {
            throw new Error('Wrong fileMeta');
          }
        } else {
          throw HTTP_STATUSES.NOT_FOUND.createError();
        }
