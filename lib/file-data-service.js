...
// eslint-disable-next-line global-require,import/no-dynamic-require
    this._storage = require(`./storages/${this.storage}`);
  ...
  selectOne(scope) {
    return this
      ...
      .then((model) => {
        ...
        return this.getFileField(fileFieldName, model);
      })
      .then((fileData) => {
...
// eslint-disable-next-line no-console
  ...
  'delete'(scope) {
    return this
      ...
      .then((model) => {
        scope.model = model;
        const fileFieldName = this.getFileFieldName(scope);
        return this.cleanFileField(fileFieldName, model);
      })
  ...
  getFileField(fieldName, model) {
    return Bb
      .try(() => {
        ...
        if (fileMeta) {
          return Bb
            .try(() => this._storage.getStream(fileMeta))
      ...
      .try(() => {
        ...
        if (file) {
          return Bb
            ...
            .then((result) => {
              const options = { content_type: result.mimeType };
              const fileMeta = this.getFileMeta(model, fieldName);
              if (fileMeta) {
                return this._storage.replaceFile(fileMeta, result.path, options);
              } else {
                return this._storage.putFile(result.path, options);
              }
            })
            .then((result) => {
  ...
  cleanFileField(fieldName, model) {
    ...
    if (fileMeta) {
      return Bb
        .try(() => this._storage.deleteFile(fileMeta))
        .then(() => {
